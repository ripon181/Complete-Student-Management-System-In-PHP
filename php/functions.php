...
class login_registration_class{
	...
	---------------------------------
	**/
	public function fct_registration($name,$uname, $pass,$email, $bday,$gender,$dept,$subt,$edu,$contact,$address){
		global $conn;
		$fct = $conn->query("select id from faculty where username='$uname' ");
		$count = $fct->num_rows;
		if($count == 0){
			$sql = "insert into faculty(name,username,password,email,birthday,gender,dept,subt,education,contact,address) values('$name','$uname','$pass','$email','$bday','$gender','$dept','$subt','$edu','$contact','$address')";
			$result = $conn->query($sql);
			return true;
		}else{
			return false;
...
public function get_faculty_username($id){
    global $conn;
    $query = $conn->query("SELECT name FROM faculty WHERE id='$id'");
    $result = $query->fetch_assoc();
    echo $result['name'];
}

// Get all info of a specific faculty member by ID
public function getfacultybyid($id){
    global $conn;
    $query = $conn->query("SELECT * FROM faculty WHERE id='$id'");
    return $query;
}

// Update Faculty Profile
public function update_faculty_profile($id, $name, $email, $dept, $subt, $gender, $contact, $address, $file) {
    global $conn;
    $query = $conn->query("UPDATE faculty SET name='$name', email='$email', dept='$dept', subt='$subt', gender='$gender', contact='$contact', address='$address', img='$file' WHERE id='$id'");
    if ($query) {
        return true;
    } else {
        return $conn->error; // Return the error message for debugging
    }
}


// Change Faculty Password
public function update_faculty_password($id, $newpass, $oldpass){
    global $conn;
    $query = $conn->query("SELECT id FROM faculty WHERE id='$id' AND password='$oldpass'");
    $count = $query->num_rows;
    if($count == 0){
        return print("<p style='color:red;text-align:center'>Old password does not match.</p>");
    } else {
        $update = $conn->query("UPDATE faculty SET password='$newpass' WHERE id='$id'");
        return print("<p style='color:green;text-align:center'>Password changed successfully.</p>");
    }
}
...
public function delete_faculty($id){
	...
	*/
	...
	public function insertattn($cur_date, $dept, $subt, $semester, $time, $atten = array()){
		global $conn;
		...
		foreach($atten as $key => $attn_value ){
			if($attn_value == "present"){
				$sql = "insert into attn(st_id, dept, subt, semester, at_time, atten, at_date) values('$key', '$dept', '$subt', '$semester', '$time', 'present', '$cur_date')";
				$att_res = $conn->query($sql);
			} elseif($attn_value == "absent"){
				$sql = "insert into attn(st_id, dept, subt, semester, at_time, atten, at_date) values('$key', '$dept', '$subt', '$semester', '$time', 'absent', '$cur_date')";
				$att_res = $conn->query($sql);
			}
		}
		if($att_res){
	...
	public function attn_all_student_by_id($st_id) {
		global $conn;
		$res = $conn->query("SELECT at_date, atten FROM attn WHERE st_id = '$st_id'");
		return $res;
	}
