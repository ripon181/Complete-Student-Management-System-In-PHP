...
<div class="all_student fix">
		...
		<div  class="fix" style="background:#ddd;padding:20px;">
			...
			<span style="text-align:center;"><a style="color:#fff;" href="st_result.php"> <button style="background:#58A85D;border:none;color:#fff;padding:10px;">Student Result</button></a></span>
		</div>
		<?php
		...
		<?php
			if($_SERVER['REQUEST_METHOD'] == 'POST'){
				...
				$dept = $_POST['dept']; // Add this line
				$subt = $_POST['subt']; // Add this line
				$semester = $_POST['semester'];
				$time = $_POST['attntime']; // Add this line
				$atten = $_POST['attn'];
				
				$time = date("h:i A", strtotime($time));

				$res = $user->insertattn($cur_date, $dept, $subt, $semester, $time, $atten);// Pass all required parameters
				if($res){
